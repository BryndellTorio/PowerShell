...
# To make the modern looking search bar.
  ...
  process
  {
    ...
    if (($_tmp[$_tmp.Length - 1] -eq "Project") -or ($_tmp[$_tmp.Length - 1] -eq "Projects") -or ($_tmp[$_tmp.Length - 1] -eq "tmp"))
    {
      ...
      } else
      {
        ...
        New-item -Name README.md -Path ".\$Name\Design Files" -ItemType File
        "Design files such as schematic capture, layout, firmware and simulations file are located here." >> ".\$Name\Design Files\README.md"
        New-item -Name README.md -Path ".\$Name\Documents" -ItemType File
        "All document files related to this project are located here." >> ".\$Name\Documents\README.md"
...
function Clear-DownloadFolder
{
  ...
  process
  {
    Remove-Item -Path $HOME\Downloads\* -Confirm -Recurse
  }
}
...
### IMPROVEMENT: Check if the file contained inside the folder is not redundant.
### i.e. 2 *.xlsx file inside or 2 .pdf file contained inside.
function Package-File
{
  ...
  {
    $tFileCheck = $Name + ".zip";
    if (Test-Path -Path $tFileCheck)
    {
      Write-Warning "I found the same filename: $tFileCheck"
    } else
    {
      if (Test-Path -Path ./$Name)
      {
        Set-Location $Name;
        $tDirectory = Get-Location;
        Set-Location ..;
      } else
      {
        $tDirectory = mkdir $Name;
      }
      $tZipName = [string]$tDirectory + ".zip";
      Copy-Item *.DSN,*.OPJ,*.DRC,*.NET,*.BOM,*.PDF,*.OLB,*.xlsx $tDirectory;
      Compress-Archive $tDirectory $tZipName;
      $tCountHandler = Get-ChildItem $tDirectory;
      $tCount = $tCountHandler.Count;
      if ($tCount -gt 8)
      { # Giving alerts depending on the number items inside the folder.
        Write-Warning "Containing $tCount items GREATER than expected.`nCheck file: $tZipName";
        Get-ChildItem $tDirectory -Name
      } elseif ($tCount -lt 7)
      {
        Write-Warning "Containing $tCount items LESS than expected.`nCheck file: $tZipName";
        Get-ChildItem $tDirectory -Name
      } else
      {
        Get-ChildItem $tDirectory -Name
